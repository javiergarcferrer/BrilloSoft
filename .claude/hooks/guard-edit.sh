...
# the UI obeys docs/IDENTIDAD.md. Blocks with the rule and where it lives.
...
if printf '%s' "$nuevo" | grep -qE "$SECRETO_VALORES"; then
  negar "looks like a server key or private key. Sensitive material lives inside Postgres, never in the app (docs/PLAN-DEMOCRACIA.md §4)."
fi
if ! es_archivo_supabase "$rel" && [ "${rel##*.}" != "md" ] && printf '%s' "$nuevo" | grep -qE "$SECRETO_NOMBRES"; then
  negar "the service role is named only inside supabase/ (grants, the Edge Function trust boundary — docs/PLAN-DEMOCRACIA.md §9.2), never on an app surface."
fi
...
if es_archivo_ui "$rel"; then
  ...
  if [ -n "$hallazgos" ]; then
    negar "docs/IDENTIDAD.md prohibits gradients, blur, glass shadows (use border-hairline; shadow-card/shadow-soft only for what truly floats), rounded-2xl/3xl (rounded-lg max) and emoji (icons are stroke icons from components/icons.tsx). Use the primitives in components/papel.tsx. Offending lines:
$hallazgos"
  fi
fi
