...
- ✅ **1.5** — `lib/seguimiento.ts` con entradas tipadas (proceso, proyecto,
  ...
  tipo en las fichas de proceso, Diputados, Senado, proveedor, norma y capítulo
  (este solo compartir: es una instantánea). ⚠️ La ficha de institución
  (`/instituciones/[id]`) no existía en este árbol: lleva
  `<AccionesFicha tipo="institucion" id=… titulo=… href=… />` al integrarse.
